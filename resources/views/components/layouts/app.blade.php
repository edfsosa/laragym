            ...
            <x-menu activate-by-route>
                ...
                @if ($user = auth()->user())
                    ...
                    <x-list-item :item="$user" sub-value="email" no-separator no-hover class="-mx-2 -my-2">
                        <x-slot:actions>
                            <x-button icon="o-power" class="btn-circle btn-ghost btn-sm"
                                tooltip-left="{{ __('Logout') }}" no-wire-navigate link="/logout" />
                        </x-slot:actions>
                    </x-list-item>
                ...
                <x-menu-item title="{{ __('Memberships') }}" icon="o-identification"
                    link="{{ route('memberships.index') }}" />
                <x-menu-item title="{{ __('Routines') }}" icon="o-clipboard-document-list"
